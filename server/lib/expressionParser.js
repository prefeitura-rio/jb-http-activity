...
const TRANSFORM_FUNCTIONS = {
    ...
    substr: (v, i, n) => {
        if (v == null)
            return null;
        const start = typeof i === 'number' ? i : Number(i) || 0;
        const length = typeof n === 'number' ? n : Number(n) || 0;
    ...
    round: (v, n) => {
        if (v == null)
            return null;
        const factor = Math.pow(10, typeof n === 'number' ? n : (Number(n) || 0));
...
function evaluateExpression(expression, responseData) {
    ...
    if (trimmed.includes('(') === false) {
        ...
        if (isDotNotation(trimmed) || isSimplePath(trimmed)) {
            ...
            if ((0, types_1.isLiteralValue)(val))
                return val;
            return null;
        }
        return trimmed;
    ...
    const args = argsRaw.map((arg) => {
        ...
        if (argTrimmed.includes('('))
            return evaluateExpression(argTrimmed, responseData);
        if (argTrimmed.startsWith("'") && argTrimmed.endsWith("'"))
            return argTrimmed.slice(1, -1);
        if (argTrimmed.startsWith('"') && argTrimmed.endsWith('"'))
            return argTrimmed.slice(1, -1);
        if (argTrimmed === 'true')
            return true;
